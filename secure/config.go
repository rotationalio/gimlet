...
type Config struct {
	...
	ContentSecurityPolicy           CSPDirectives     `split_words:"true" desc:"Value for the Content-Security-Policy header. If empty, the header is not added."`
	ContentSecurityPolicyReportOnly CSPDirectives     `split_words:"true" default:"false" desc:"If true, the Content-Security-Policy-Report-Only header is added instead of Content-Security-Policy."`
	ReportingEndpoints              map[string]string `split_words:"true" desc:"Reporting endpoints for reports generated by the Reporting API such as CSP violations or cross-origin-opener policy reports. The map key is the endpoint name, the value is the URL."`
	HSTS                            HSTSConfig        `split_words:"true" desc:"Configuration for the HTTP Strict Transport Security header"`
}
...
func (c Config) Validate() (err error) {
	...
	if !c.ContentSecurityPolicyReportOnly.IsZero() && len(c.ReportingEndpoints) == 0 {
		err = errors.Join(err, errors.New("reporting endpoints must be set when using a csp reporting-only policy"))
	}
